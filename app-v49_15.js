...
/* ===== V49.15-LC5 LEGACY CUT MOCKUP BUILDER =====
...
function renderMockupBuilder(){
  ...
  <div class="mockupBuilder">
    <div class="mockupControls box">
      ...
      <label>Custom Base Color</label><input type="color" id="mockBaseColor" value="${escapeHtml(m.customBaseColor)}" oninput="updateMockupField('customBaseColor',this.value)">
      ...
      <label>Words</label><input id="mockText" value="${escapeHtml(m.draftText)}" oninput="updateMockupField('draftText',this.value)">
      <div class="two"><div><label>Font</label><select id="mockFont" onchange="updateMockupField('draftFont',this.value)">${mockupFontOptions(m.draftFont)}</select></div><div><label>Text Size</label><input type="number" min="18" max="220" value="${escapeHtml(m.draftSize)}" oninput="updateMockupField('draftSize',this.value)"></div></div>
      <div class="two"><div><label>Letter Stain / Color</label><select id="mockTextFinish" onchange="updateMockupField('draftFinish',this.value)">${mockupFinishOptions(m.draftFinish)}</select></div><div><label>Custom Letter Color</label><input type="color" value="${escapeHtml(m.draftCustomColor)}" oninput="updateMockupField('draftCustomColor',this.value)"></div></div>
  ...
  <div class="mockupSaved box"><div class="titleRow miniTitle"><div><h3>Saved Mockups</h3><p class="note">Saved mockups stay in this Legacy Cut copy. Gallery saves also appear under Studio → Gallery.</p></div></div>${renderSavedMockups()}</div>`;
}
...
function renderSelectedMockupEditor(layer){return `<div class="selectedMockupEditor"><h3>Selected Text Piece</h3><label>Text</label><input value="${escapeHtml(layer.text||'')}" oninput="updateMockupLayer('${layer.id}','text',this.value)"><div class="two"><div><label>Font</label><select onchange="updateMockupLayer('${layer.id}','font',this.value)">${mockupFontOptions(layer.font)}</select></div><div><label>Size</label><input type="number" value="${escapeHtml(layer.size||80)}" oninput="updateMockupLayer('${layer.id}','size',this.value)"></div></div><div class="two"><div><label>Finish</label><select onchange="updateMockupLayer('${layer.id}','finish',this.value)">${mockupFinishOptions(layer.finish||'dark')}</select></div><div><label>Custom Color</label><input type="color" value="${escapeHtml(layer.customColor||'#5a321c')}" oninput="updateMockupLayer('${layer.id}','customColor',this.value)"></div></div><label>Rotate</label><input type="range" min="-45" max="45" value="${escapeHtml(layer.rotate||0)}" oninput="updateMockupLayer('${layer.id}','rotate',this.value)"></div>`}
