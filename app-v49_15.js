...
function refreshSupplyTrackers(id){
 ...
 let unitEl=document.getElementById('sPricePerUnit'); if(unitEl)unitEl.value=item.pricePerUnit||'';
...
/* LEGACY CUT MERIDIAN PATCH v1.0
...
function updateLegacyCutCheckbox(id,group,value,checked){
 ...
 normalizeInvoice(inv,{skipSupplySync:true});save('legacy-cut-checkbox');
}
function setLegacyCutPaymentMethod(id,method,checked){
 ...
 if(document.getElementById('clientInvoiceEmbed'))refreshClientInvoiceEmbed(id);else renderInvoiceEditor(id);
}
function setLegacyCutTotalDue(id,value){
 const inv=state.invoices.find(i=>i.id===id);if(!inv)return;
 const amount=normalizeMoneyNumber(value||0,2);
...
function legacyCutOrderDetails(inv){
 const lc=ensureLegacyCutInvoice(inv);
 const lines=[];
 const products=[...lc.products]; if(lc.productOther)products.push('Other: '+lc.productOther);
...
function legacyCutOrderFormHtml(inv,mode='full'){
 ...
 return `<div class="legacyOrderForm">
  ...
  <label>TOTAL PRICE DUE</label><input type="number" step="0.01" value="${Number(inv.total||0).toFixed(2)}" oninput="setLegacyCutTotalDue('${inv.id}',this.value)">
  <label>TOTAL PAID</label><input type="number" step="0.01" value="${Number(inv.paid||0).toFixed(2)}" oninput="updateInvoicePaid('${inv.id}',this.value)" onblur="${mode==='client'?`refreshClientInvoiceEmbed('${inv.id}')`:`renderInvoiceEditor('${inv.id}')`}">
