...
const MOCKUP_PRODUCT_TEMPLATES={
  ...
  garden:{label:'Garden Stakes',shape:'gardenStake',width:3.5,height:18,thickness:.125},
...
function mockupPuzzleOverlaySvg(){const map={20:[4,5],50:[5,10],100:[10,10],500:[18,22],1000:[24,30]};const rc=map[String(state.mockupBuilder?.puzzlePieces||'20')]||map[20];const rows=rc[0],cols=rc[1];let parts='';const x=240,y=145,w=520,h=400;for(let c=1;c<cols;c++){const px=x+(w/cols)*c;parts+=`<line x1="${px}" y1="155" x2="${px}" y2="535" stroke="rgba(78,49,20,.18)" stroke-width="2"/>`;}for(let r=1;r<rows;r++){const py=y+(h/rows)*r;parts+=`<line x1="250" y1="${py}" x2="750" y2="${py}" stroke="rgba(78,49,20,.18)" stroke-width="2"/>`;}parts+=`<text x="500" y="575" text-anchor="middle" font-size="28" font-family="Arial, Helvetica, sans-serif" fill="rgba(78,49,20,.55)">${state.mockupBuilder?.puzzlePieces||'20'} piece preview</text>`;return parts;}
