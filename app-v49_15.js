...
const SECTIONS={
 ...
 supplies:{sub:'Supplies / Supply List',tabs:[['list','Supplies List','Raw materials, blanks, paint, stain, and receipt folders.'],['productInventory','Product Inventory','Finished projects/products ready to sell.']]},
...
function renderSupplyReceipts(){
 ensureCollections();
 let folders=state.supplyReceiptFolders||[];
 if(!folders.length){folders=[{id:'default',name:'General Receipts'}];state.supplyReceiptFolders=folders;}
 let current=folders.find(f=>f.id===state.selectedReceiptFolderId)||folders[0];
 state.selectedReceiptFolderId=current.id;
 let receipts=(state.supplyReceipts||[]).filter(r=>(r.folderId||'default')===current.id);
 return `<div class="titleRow"><div><h2>Receipt Folders</h2><p>Organize supply receipts into named folders.</p></div><div class="actions"><button class="save" onclick="newSupplyReceipt()">+ Add New Receipt</button><button onclick="backToSupplyList()">Supplies List</button></div></div><div class="box"><div class="actions"><input id="newReceiptFolderName" placeholder="New folder name"><button type="button" onclick="createReceiptFolder()">Create Folder</button></div><div class="receiptFolderGrid">${folders.map(f=>`<button type="button" class="${f.id===current.id?'active':''}" onclick="selectReceiptFolder('${f.id}')">${escapeHtml(f.name)}</button>`).join('')}</div><div class="two"><div><label>Selected Folder Name</label><input id="selectedReceiptFolderName" value="${escapeHtml(current.name)}"></div><div class="actions"><button type="button" onclick="renameReceiptFolder('${current.id}')">Rename Folder</button>${current.id==='default'?'':`<button type="button" class="delete" onclick="deleteReceiptFolder('${current.id}')">Delete Folder</button>`}</div></div></div><div class="clientList">${receipts.map(r=>`<div class="invoiceCard" onclick="openSupplyReceipt('${r.id}')"><b>${escapeHtml(r.title||'Receipt')}</b><small>${escapeHtml(r.date||'')} • ${escapeHtml(r.category||'Uncategorized')} • ${money(r.amount)}</small></div>`).join('')||'<p class="note">No receipts in this folder yet. Tap + Add New Receipt.</p>'}</div>`
}
function createReceiptFolder(){ensureCollections();let name=String(document.getElementById('newReceiptFolderName')?.value||'').trim();if(!name){alert('Name the folder first.');return;}let id=uid();state.supplyReceiptFolders.push({id,name});state.selectedReceiptFolderId=id;save('receipt-folder-create');render()}
function selectReceiptFolder(id){ensureCollections();state.selectedReceiptFolderId=id||'default';save('receipt-folder-select');render()}
function renameReceiptFolder(id){ensureCollections();let f=state.supplyReceiptFolders.find(x=>x.id===id);if(!f)return;let name=String(document.getElementById('selectedReceiptFolderName')?.value||'').trim();if(!name){alert('Folder name cannot be blank.');return;}f.name=name;save('receipt-folder-rename');render()}
function deleteReceiptFolder(id){ensureCollections();if(id==='default')return;if(!confirm('Delete this folder? Receipts inside will move to General Receipts.'))return;(state.supplyReceipts||[]).forEach(r=>{if((r.folderId||'default')===id)r.folderId='default'});state.supplyReceiptFolders=state.supplyReceiptFolders.filter(f=>f.id!==id);state.selectedReceiptFolderId='default';save('receipt-folder-delete');render()}
...
function saveSupplyItem(id){
 ...
 state.supplyListMode='list';
 goToFirstTab('supplies');
}
function newSupplyItem(){ensureCollections();createBlankSupplyItemDraft();state.section='supplies';state.tabs.supplies='list';state.supplyListMode='item';save('new-supply-item');render()}
...
function addQuickSupplyFromList(){
 ...
 if(existing){state.selectedSupplyId=existing.id;state.section='supplies';state.tabs.supplies='list';state.supplyListMode='item';save('quick-supply-existing');render();return;}
 let id=uid();
 let number=nextSupplyNumber();
...
function openSupplyItem(id){state.selectedSupplyId=id;state.section='supplies';state.tabs.supplies='list';state.supplyListMode='item';save('open-supply-item');render()}
...
function addInventoryToSupply(id){
 ...
 state.section='supplies';state.tabs.supplies='list';state.supplyListMode='item';state.selectedSupplyId=id;
 render();
}
function formatQty(q,unit){if(q===''||q===undefined||q===null)return'—';return `${Number(q||0)} ${unit||'unit'}`}
function newSupplyReceipt(){ensureCollections();let id=uid();let r={id,title:'New Receipt',date:new Date().toLocaleDateString(),category:'Yard Supplies',amount:0,image:'',items:[],folderId:state.selectedReceiptFolderId||'default'};state.supplyReceipts.push(r);save('new-supply-receipt');openSupplyReceipt(id)}
function openSupplyReceipt(id){let r=state.supplyReceipts.find(x=>x.id===id);if(!r)return;content.innerHTML=`<div class="titleRow"><div><h2>${escapeHtml(r.title||'Receipt')}</h2><p>${escapeHtml(r.date||'')} • ${escapeHtml(r.category||'')}</p></div><button onclick="openSupplyReceiptsPage()">Back</button></div><div class="box"><label>Receipt Name</label><input id="receiptTitle" value="${escapeHtml(r.title||'')}" oninput="updateReceipt('${id}','title',this.value)"><label>Date</label><input id="receiptDate" value="${escapeHtml(r.date||'')}" oninput="updateReceipt('${id}','date',this.value)"><label>Category</label><input id="receiptCategory" value="${escapeHtml(r.category||'')}" oninput="updateReceipt('${id}','category',this.value)"><label>Folder</label><select onchange="updateReceipt('${id}','folderId',this.value)">${(state.supplyReceiptFolders||[{id:'default',name:'General Receipts'}]).map(f=>`<option value="${f.id}" ${(r.folderId||'default')===f.id?'selected':''}>${escapeHtml(f.name)}</option>`).join('')}</select><label>Total Price</label><input id="receiptAmount" type="number" step="0.01" value="${Number(r.amount||0)}" oninput="updateReceipt('${id}','amount',this.value)"><label>Receipt Photo</label><input type="file" accept="image/*" capture="environment" onchange="attachReceiptPhoto(event,'${id}')">${r.image?`<img class="photo" src="${r.image}">`:''}<div class="actions"><button class="save" onclick="saveSupplyReceiptAndReturn('${id}')">Save Receipt</button><button onclick="openSupplyReceiptsPage()">Back</button><button class="delete" onclick="deleteReceipt('${id}')">Delete</button></div></div>`}
function saveSupplyReceiptAndReturn(id){const r=state.supplyReceipts.find(x=>x.id===id);if(r){if(!r.folderId)r.folderId=state.selectedReceiptFolderId||'default';adminRecord?.('supply.receipt.save',`Supply receipt saved: ${r.title||'Receipt'}`,{date:r.date,amount:r.amount,category:r.category,folderId:r.folderId,hasImage:!!r.image});state.selectedReceiptFolderId=r.folderId||state.selectedReceiptFolderId||'default';}save('supply-receipt-save-return');openSupplyReceiptsPage()}
...
function deleteReceipt(id){state.supplyReceipts=state.supplyReceipts.filter(r=>r.id!==id);save('receipt-delete');openSupplyReceiptsPage()}
...
function renderProductInventory(){
 ...
 return `<div class="titleRow"><div><h2>Product Inventory</h2><p>Finished projects/products ready to sell. This is separate from raw supplies.</p></div><button class="save" type="button" onclick="addFinishedProduct()">+ Add Product</button></div><div class="clientList">${products.map(p=>`<div class="invoiceCard"><b>${escapeHtml(p.name||'Untitled Product')}</b><small>${escapeHtml(p.type||'')} • ${escapeHtml(p.size||'')} • Available: ${finishedProductAvailable(p)} • Price: ${money(p.finalPrice||p.basePrice||0)}</small><div class="actions"><button type="button" onclick="editFinishedProduct('${p.id}')">Edit</button><button type="button" class="delete" onclick="deleteFinishedProduct('${p.id}')">Delete</button></div></div>`).join('')||'<p class="note">No finished products yet. Tap + Add Product to add ready-to-sell items.</p>'}</div><div id="finishedProductEditor"></div>`
}
function finishedProductAvailable(p){return Math.max(0,Number(p.qtyMade||0)-Number(p.qtySold||0))}
function addFinishedProduct(){ensureCollections();let p={id:uid(),name:'',type:'',size:'',materials:'',qtyMade:0,qtySold:0,basePrice:0,addonPrice:0,finalPrice:0,notes:''};state.finishedProducts.unshift(p);save('finished-product-add');render();setTimeout(()=>editFinishedProduct(p.id),30)}
function editFinishedProduct(id){ensureCollections();let p=state.finishedProducts.find(x=>x.id===id);if(!p)return;let el=document.getElementById('finishedProductEditor');if(!el)return;el.innerHTML=`<div class="box"><div class="titleRow miniTitle"><div><h3>${p.name?escapeHtml(p.name):'New Product'}</h3><p class="note">Finished sellable product details.</p></div><button onclick="closeFinishedProductEditor()">Close</button></div><label>Product Name</label><input value="${escapeHtml(p.name||'')}" oninput="updateFinishedProduct('${id}','name',this.value)"><label>Product Type</label><input value="${escapeHtml(p.type||'')}" placeholder="Sign, coaster, keychain, bookmark..." oninput="updateFinishedProduct('${id}','type',this.value)"><label>Size</label><input value="${escapeHtml(p.size||'')}" placeholder='Example: 6&quot; × 4&quot;' oninput="updateFinishedProduct('${id}','size',this.value)"><label>Materials / Supplies Used</label><textarea oninput="updateFinishedProduct('${id}','materials',this.value)">${escapeHtml(p.materials||'')}</textarea><div class="three"><div><label>Quantity Made</label><input type="number" step="1" value="${Number(p.qtyMade||0)}" oninput="updateFinishedProduct('${id}','qtyMade',this.value)"></div><div><label>Quantity Sold</label><input type="number" step="1" value="${Number(p.qtySold||0)}" oninput="updateFinishedProduct('${id}','qtySold',this.value)"></div><div><label>Available</label><input value="${finishedProductAvailable(p)}" readonly></div></div><div class="three"><div><label>Base Price</label><input type="number" step="0.01" value="${Number(p.basePrice||0)}" oninput="updateFinishedProduct('${id}','basePrice',this.value)"></div><div><label>Add-on Pricing</label><input type="number" step="0.01" value="${Number(p.addonPrice||0)}" oninput="updateFinishedProduct('${id}','addonPrice',this.value)"></div><div><label>Final Sell Price</label><input type="number" step="0.01" value="${Number(p.finalPrice||0)}" oninput="updateFinishedProduct('${id}','finalPrice',this.value)"></div></div><label>Notes</label><textarea oninput="updateFinishedProduct('${id}','notes',this.value)">${escapeHtml(p.notes||'')}</textarea><div class="actions"><button class="save" onclick="saveFinishedProduct('${id}')">Save Product</button><button onclick="closeFinishedProductEditor()">Close</button></div></div>`;el.scrollIntoView({behavior:'smooth',block:'nearest'});}
function updateFinishedProduct(id,key,value){let p=state.finishedProducts.find(x=>x.id===id);if(!p)return;if(['qtyMade','qtySold','basePrice','addonPrice','finalPrice'].includes(key))value=Number(value||0);p[key]=value;if(key==='basePrice'||key==='addonPrice')p.finalPrice=normalizeMoneyNumber(Number(p.basePrice||0)+Number(p.addonPrice||0),2);save('finished-product-update')}
function saveFinishedProduct(id){save('finished-product-save');render()}
function closeFinishedProductEditor(){render()}
function deleteFinishedProduct(id){if(!confirm('Delete this finished product?'))return;state.finishedProducts=(state.finishedProducts||[]).filter(p=>p.id!==id);save('finished-product-delete');render()}
