...
/* ===== V49.15-LC10 LEGACY CUT MOCKUP BUILDER =====
...
function renderMockupBuilder(){
  ensureMockupBuilder();const m=state.mockupBuilder;const sel=m.layers.find(l=>l.id===m.selectedLayerId)||null;const editor=mockupTextEditorState();
  ...
  <div class="mockupBuilder">
    <div class="mockupControls box">
      ...
      <label>Custom Base Color</label><button type="button" class="mockColorButton" onclick="openMockupColorPicker('base')"><span class="mockColorSwatch" style="background:${escapeHtml(m.customBaseColor||'#d9b575')}"></span><span>Open Color Wheel</span></button>
      <div class="three"><div><label>Width</label><input type="number" step="0.25" value="${escapeHtml(m.width)}" oninput="updateMockupFieldLive('width',this.value);updateMockupStageLabelLive()" onchange="render()"></div><div><label>Height</label><input type="number" step="0.25" value="${escapeHtml(m.height)}" oninput="updateMockupFieldLive('height',this.value);updateMockupStageLabelLive()" onchange="render()"></div><div><label>Thickness</label><input type="number" step="0.125" value="${escapeHtml(m.thickness)}" oninput="updateMockupFieldLive('thickness',this.value);updateMockupStageLabelLive()" onchange="render()"></div></div>
      <h3>Text Cutout</h3>
      <p class="note">One text area: type words, choose font/color, then add or edit the selected cutout.</p>
      <label>Cutout Words</label><input id="mockText" value="${escapeHtml(editor.text)}" oninput="updateMockupTextEditor('text',this.value)">
      <div class="two"><div><label>Font</label><select id="mockFont" onchange="updateMockupTextEditor('font',this.value)">${mockupFontOptions(editor.font)}</select></div><div><label>Text Size</label><input type="number" min="18" max="220" value="${escapeHtml(editor.size)}" oninput="updateMockupTextEditor('size',this.value)"></div></div>
      <div class="two"><div><label>Letter Stain / Color</label><select id="mockTextFinish" onchange="updateMockupTextEditor('finish',this.value)">${mockupFinishOptions(editor.finish)}</select></div><div><label>Custom Cutout Color</label><button type="button" class="mockColorButton" onclick="openMockupColorPicker('${sel?'layerText':'draftText'}','${sel?sel.id:''}')"><span class="mockColorSwatch" style="background:${escapeHtml(editor.customColor||'#5a321c')}"></span><span>Open Color Wheel</span></button></div></div>
      ${sel?`<label>Rotate Selected Cutout</label><input type="range" min="-45" max="45" value="${escapeHtml(sel.rotate||0)}" oninput="updateMockupLayerLive('${sel.id}','rotate',this.value)">`:''}
      <div class="actions"><button type="button" class="save" onclick="addMockupTextLayer()">Add Text Cutout</button>${sel?`<button type="button" onclick="clearMockupSelection()">New Text</button><button type="button" onclick="duplicateSelectedMockupLayer()">Duplicate Selected</button><button type="button" class="delete" onclick="deleteSelectedMockupLayer()">Delete Selected</button>`:''}</div>
      <p class="note">${sel?'Editing selected text cutout. Tap New Text to stop editing and create a fresh cutout.':'No cutout selected. The controls above will create a new text cutout.'}</p>
    ...
    <div class="mockupStage box">
      <div class="mockupStageHeader"><b>${escapeHtml((MOCKUP_PRODUCT_TEMPLATES[m.product]||{}).label||'Custom Product')}</b><small id="mockupStageDims">${escapeHtml(m.width)}&quot; × ${escapeHtml(m.height)}&quot; × ${escapeHtml(m.thickness)}&quot;</small></div>
      ${renderMockupSvg()}
      <p class="note">Drag text cutouts directly on the wood. Tap a cutout to edit it with the one text panel.</p>
    </div>
  </div>
...
function mockupFontList(){return [
 ...
 {label:'Trebuchet',family:'"Trebuchet MS", Arial, sans-serif'},
 {label:'Impact',family:'Impact, Haettenschweiler, sans-serif'},
 {label:'Courier New',family:'"Courier New", Courier, monospace'},
 {label:'Handwriting Script',family:'"Brush Script MT", "Segoe Script", "Lucida Handwriting", cursive'},
...
function mockupStagePointerDown(evt){if(evt.target.closest&&evt.target.closest('.mockTextLayer'))return;if(mockupDrag)return;if(state.mockupBuilder?.selectedLayerId){state.mockupBuilder.selectedLayerId='';save('mockup-stage-deselect');render()}}
